...
error_collector = []
global logfile
logfile= 'ssh-exec-commands-verify-log.txt'
global fo
fo = open(logfile, 'w+')
plan_to_verify = {
    "1" : {
    ...
    "5" : {
        ...
        "username" : "usr5",
        "password" : "pwd5",
        "commands" : ["CHECK_PING::10.10.1.21", "CHECK_PING::10.10.1.27", 
            "CHECK_PING::10.64.104.43", 
            ],
    },
    "6" : {
        "hostname": "host6",
        ...
        "username" : "usr6",
        "password" : "pwd6",
        "commands" : ["DIR_EXISTS::/test/latest/abc.py"],
    },
}
...
def connect_ssh(hostname, username, password):
    ...
    try:
        ...
        print('Connected: %s' % hostname)
        return (1)
    except Exception as ex:
        print("*** ERROR:: Failed to connect the node: %s : %s ***" % (hostname, str(ex)))
...
def disconnect_ssh(hostname, ip):
    client.close()
    print("Connection Closed: %s [%s]" % (hostname, ip))
...
def check_disk_space(grepOption):
    ...
    #print(_stdin.read().decode())
    stdout = _stdout.read().decode()
    stderr = _stderr.read().decode()
    if stderr:
        print("*** ERROR:: Error received from the command: %s on executing on client: %s" % (command, clientHostname))
        ...
        print("usedSpacePercentage = %s" % usedSpacePercentage)
...
def check_free_memory():
    ...
    print("stdout = \'%s\'" % stdout.strip())
    if not stdout:
        print("*** ERROR:: No output received from the command: %s on executing on client: %s" % (command, clientHostname))
    ...
    if stderr:
        ...
        error_collector.append("*** ERROR:: Error received from the command: %s on executing on client: %s" % (command, clientHostname))
        print("stderr = %s" % stderr)
    print("availableMemory = %s" % availableMemory)
...
def check_ip_pings(ipToPing):
    ...
    if stderr:
        ...
        print("packetLossPercentage = %s" % packetLossPercentage)
...
def send_email():
    ...
    email_to = 'ajaysingh002@gmail.com, ajaysingh002@yahoo.com'
    email_subject = "SSH Exec Commands Verify Tool Email"
